...
process REFORMATFUSVIZ {
	...
	input:
		val(samples)
		tuple val(samples), file(longgf_out), file(genion_out), file(ctat_out), file(fusionseeker_out)
	output:
	...
	merge_reformat.py ${samples} ${samples}_fusviz_in.tsv ${samples}_longgf_reformat.tsv ${samples}_genion_reformat.tsv ${samples}_ctat_reformat.tsv ${samples}_fusionseeker_reformat.tsv
	"""
}
