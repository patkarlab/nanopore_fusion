...
//workflow NANOPORE_FUSION {
//
//    // Detect latest ShellScript_* directory
//    def seq_base = file(params.sequences)
//    def latest_seq = seq_base
//        .listFiles()
//        .findAll { it.isDirectory() && it.name.startsWith('ShellScript_') }
//        .sort { -it.lastModified() }
//        ?.first()

//    if( !latest_seq ) {
//        error "No ShellScript_* directory found inside ${seq_base}"
//    }

//    println "Using sequence folder: ${latest_seq}"
...
//        .map { row -> row[0] }
//        .map { sample ->
//            def fq = file("${latest_seq}/${sample}.fastq.gz", checkIfExists: true)
//            tuple(sample, fq)
//        }
//        .set { samples_ch }
